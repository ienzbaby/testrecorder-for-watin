...
//取单选框的值
$.getRadioGroupValue = function(RadioGroupName) {
    var str = "";
    $($("input:radio[@name='" + RadioGroupName + "']")).each(function() {
        if (this.name == RadioGroupName && this.checked == true) {
            str = this.value;
        }
    });
    return str;
...
//===选中复选框===
$.getCheckGroupValue = function(RadioGroupName) {
    var str = "";
    $($("input:checkbox[@name='" + RadioGroupName + "']")).each(function() {
        if (this.name == RadioGroupName && this.checked == true) {
            str += this.value + ",";
        }
    });
...
//===选中单选框===
$.setRadioGroupValue = function(RadioGroupName, SetValue) {
    $($("input:radio[@name='" + RadioGroupName + "']")).each(function() {
        if (this.name == RadioGroupName && this.value == SetValue) {
            this.checked = true;
        }
    });
};
//===设置选中复选框===
$.setCheckGroupValue = function(RadioGroupName, SetValue) {
    $($("input:checkbox[@name='" + RadioGroupName + "'][@checked='checked']")).each(function() {
        this.checked = false; ;
    });
    var strs = new Array(); //定义一数组
    strs = SetValue.split(","); //字符分割   

    $($("input:checkbox[@name='" + RadioGroupName + "']")).each(function() {
        for (i = 0; i < strs.length; i++) {
            if (this.value == strs[i]) {
                this.checked = true;
            }
        }
    });
};
$.bindTemplate = function(source, opts) {
        ...
        default:
            {
                ...
                for (var key in obj) {
                    ...
                    sumSerialize += key + ':' + subserialize + ','
                }
                if (sumSerialize.charAt(sumSerialize.length - 1) == ',') {
                    sumSerialize = sumSerialize.substr(0, sumSerialize.length - 1)
                }
                sumSerialize += '}';
                return sumSerialize;
            }
